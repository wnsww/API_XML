...
public class Pagination {
	...
	public String getPagination() {
		...
		if(this.getBlockStart() > 1) {
			buffer.append("<span><a href=\"" + this.url + "?page=" + this.getBlockPrev() +"&keyword="+keyword +"&keyOption="+keyOption+"\"><</a></span>");
		}
		for(int i = this.getBlockStart(); i <= this.getBlockEnd(); i++) {
			if(page == i)
				buffer.append("<span>" + i + "</span>");
			else
				buffer.append("<span><a href=\"" + this.url + "?page=" + i +"&keyword="+keyword +"&keyOption="+keyOption+"\">" + i + "</a></span>");
		}
		if(this.getBlockEnd() < endPage) {
			buffer.append("<span><a href=\"" + this.url + "?page=" + this.getBlockNext() +"&keyword="+keyword +"&keyOption="+keyOption+ "\">></a></span>");
		}
		buffer.append("</div>");
